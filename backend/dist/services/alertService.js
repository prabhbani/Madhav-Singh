...
import { OPEN_ALERT_STATUSES, deriveCurrentMilestone, deriveDepartmentWorkload, deriveMetrics, deriveOpenMilestones, projectInclude, readMetricsFromSnapshot, toNumber, } from './caseSnapshot.js';
...
const OFFICER_STATUSES = ['ACKNOWLEDGED', 'IN_PROGRESS', 'RESOLVED', 'DISMISSED'];
...
const NOTIFICATION_FOR_DECISION = {
 ...
 */
const deriveResponsibleOfficers = async (project) => {
    ...
    for (const { score: _score, ...officer } of scored) {
        if (!best.has(officer.department))
            best.set(officer.department, officer);
    }
    return [...best.values()];
};
/** Past predictions, oldest first, for the trend and jump detectors. */
const derivePredictionHistory = (project) => project.predictions
    .slice()
 ...
 */
const deriveObservationHistory = (project) => project.predictions
    .slice()
    .sort((left, right) => left.predictedAt.getTime() - right.predictedAt.getTime())
...
const buildPredictionContext = async (project, horizonDays, asOfAt) => {
    ...
    if (stored) {
        return {
            ...
            confidenceBand: stored.confidenceBand ?? null,
            predictionStatus: 'OK',
            modelVersion: stored.modelVersion,
            predictedAt: stored.predictedAt,
        };
    }
    const live = (await predictionService.predict(project.id, horizonDays, asOfAt));
    return {
        riskLevel: (typeof live.riskLevel === 'string' ? live.riskLevel : 'LOW'),
        delayProbability: toNumber(live.delayProbability) ?? 0,
        expectedDelayDays: toNumber(live.expectedDelayDays) ?? null,
        horizonDays,
        confidenceBand: typeof live.confidenceBand === 'string' ? live.confidenceBand : 'LOW',
        predictionStatus: live.predictionStatus === 'RULE_ONLY_FALLBACK' || live.predictionStatus === 'STALE' || live.predictionStatus === 'DEGRADED'
...
export const buildAlertInput = async (projectId, options = {}) => {
    ...
    const existingAlerts = storedAlerts.map((alert) => ({
        alertId: alert.id,
 ...
 * row on every run.
 */
const movedMaterially = (alertType, previous, current) => {
    const policy = DETECTOR_POLICY[alertType];
    if (!policy)
        return true;
...
const persistAlert = async (projectId, alert, result, modelVersion, predictionId) => {
    ...
    const status = !existing || reopens || TERMINAL_STATUSES.includes(existing.status) ? 'OPEN' : existing.status;
    const data = {
        severity,
        status,
    ...
    const worthRecording = alert.decision !== 'UNCHANGED' || movedMaterially(alert.alertType, existing?.evidence, alert.evidence.measuredValue);
    if (worthRecording) {
        await prisma.alertEvent.create({
            data: {
                alertId: saved.id,
                eventType: EVENT_FOR_DECISION[alert.decision],
                severityBefore: existing?.severity ?? null,
...
const persistResolution = async (projectId, alertType, reason, asOfAt) => {
...
export const alertService = {
    ...
    async evaluate(projectId, options = {}) {
        ...
        if (options.notify !== false && result.notifications.length > 0) {
            const events = result.notifications.map((alert) => ({
     ...
     */
    async evaluateAll(scope, options = {}) {
        ...
        for (const project of projects) {
            try {
                ...
                runs.push({ projectId: project.id, created: 0, escalated: 0, notified: 0, error: error.message });
            }
        }
        return {
     ...
     */
    async notificationCentre(scope, filter = {}) {
        const where = {
    ...
    async acknowledge(alertId, actorId, note) {
    ...
    async updateStatus(alertId, status, actorId, note) {
    ...
    async recentActivity(scope, limit = 100) {
        const events = await prisma.alertEvent.findMany({
