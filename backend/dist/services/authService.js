    ...
    async login(request, email, password) {
        ...
        const fail = async (reason) => {
            if (user && !locked) {
                ...
                if (reachedLimit)
                    reason = `${reason}; account locked for ${env.LOGIN_LOCKOUT_MINUTES} minutes after ${env.LOGIN_MAX_FAILURES} failures`;
            }
        ...
        if (!user)
            await fail('No account for this email');
        if (locked)
            await fail('Account is locked after repeated failures');
        if (!user.active)
            await fail('Account is inactive');
        if (!passwordMatches)
            await fail('Incorrect password');
        const account = user;
        await prisma.user.update({
            where: { id: account.id },
