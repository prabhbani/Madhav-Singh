...
export const OPEN_ALERT_STATUSES = ['OPEN', 'ACKNOWLEDGED', 'IN_PROGRESS'];
export const OPEN_RECOMMENDATION_STATUSES = ['OPEN', 'ACKNOWLEDGED', 'ASSIGNED', 'IN_PROGRESS'];
...
export const ageInDays = (from, asOfAt) => from ? Math.max(0, Math.floor((asOfAt.getTime() - from.getTime()) / MS_PER_DAY)) : undefined;
/** Metrics the simplified operational schema can evidence from its own rows. */
export const deriveMetrics = (project, asOfAt) => {
    ...
    const verificationPending = documents.filter((document) => document.status === 'SUBMITTED' && document.verifiedAt === null).length;
    const openMilestones = project.milestones.filter((milestone) => milestone.status !== 'COMPLETED' && milestone.status !== 'CANCELLED');
    const overdueMilestones = openMilestones.filter((milestone) => milestone.status === 'OVERDUE' || milestone.plannedAt.getTime() < asOfAt.getTime());
    ...
    const averageStageDurationDays = completed.length > 0
        ? Math.round(completed.reduce((total, milestone) => {
 ...
 */
export const readMetricsFromSnapshot = (snapshot) => {
...
export const deriveCurrentMilestone = (project, asOfAt) => {
...
export const deriveOpenMilestones = (project) => project.milestones
...
export const derivePendingTasks = (project, asOfAt) => {
    const tasks = [];
    const push = (taskCode, label, rows) => {
        if (rows.length === 0)
            return;
    ...
    const byStatus = (statuses) => project.documents.filter((document) => statuses.includes(document.status));
    ...
    push('DOCUMENT_VERIFICATION_PENDING', 'Submitted documents awaiting verification', project.documents.filter((document) => document.status === 'SUBMITTED' && document.verifiedAt === null));
    return tasks;
};
/**
 ...
 */
export const deriveDepartmentWorkload = async (asOfAt) => {
    ...
    for (const alert of openAlerts)
        add(alert.responsibleDepartment || alert.project.department);
    for (const recommendation of openRecommendations)
        add(recommendation.responsibleDepartment);
    for (const milestone of overdueMilestones)
        add(milestone.ownerDept ?? milestone.project.department);
    const capacityByDepartment = new Map();
    for (const row of activeProjects)
        capacityByDepartment.set(row.department, row._count.id);
    for (const department of openItems.keys())
        if (!capacityByDepartment.has(department))
            capacityByDepartment.set(department, 0);
    const workload = [];
    for (const [department, activeCount] of capacityByDepartment) {
        const items = openItems.get(department) ?? 0;
        const capacity = Math.max(WORKLOAD_DERIVATION.baselineCapacityItems, activeCount * WORKLOAD_DERIVATION.openItemsPerActiveProjectCapacity);
        workload.push({
            department,
...
export const deriveHistoricalPatterns = async (project, asOfAt) => {
    const rate = async (where) => {
...
export const readRiskFactors = (explanation) => {
    const source = isRecord(explanation) ? explanation.topRiskFactors : undefined;
    if (!Array.isArray(source))
        return [];
    const factors = [];
    for (const raw of source) {
        if (!isRecord(raw) || typeof raw.factorCode !== 'string')
            continue;
        factors.push({
            factorCode: raw.factorCode,
            label: typeof raw.label === 'string' ? raw.label : undefined,
            direction: raw.direction === 'INCREASES_RISK' || raw.direction === 'REDUCES_RISK' || raw.direction === 'NEUTRAL'
                ? raw.direction
                : undefined,
