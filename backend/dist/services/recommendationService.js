...
import { OPEN_RECOMMENDATION_STATUSES, deriveCurrentMilestone, deriveDepartmentWorkload, deriveHistoricalPatterns, deriveMetrics, derivePendingTasks, projectInclude, readMetricsFromSnapshot, readRiskFactors, toNumber, } from './caseSnapshot.js';
...
export const buildRecommendationInput = async (projectId, options = {}) => {
    ...
    const existingRecommendations = project.recommendations.map((row) => ({
...
const persistRecommendation = async (projectId, recommendation, result, modelVersion, predictionId) => {
    const data = {
...
export const recommendationService = {
    ...
    list: (projectId) => prisma.recommendation.findMany({
        where: { projectId },
    ...
    async updateStatus(id, status) {
        ...
        if (!existing)
            throw new AppError(404, 'RECOMMENDATION_NOT_FOUND', 'Recommendation was not found');
