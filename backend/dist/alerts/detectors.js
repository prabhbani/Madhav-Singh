...
const fromLadder = (type, value, parts) => {
...
export const DETECTORS = [
    {
        ...
        detect: (context) => {
            ...
            return fromLadder('MILESTONE_DEADLINE_APPROACHING', daysRemaining, {
                ...
                expression: (severity, threshold) => `days_to_milestone_deadline = ${daysRemaining} <= ${threshold} (${severity} rung, milestone "${next.name}")`,
        ...
        description: (condition) => `Milestone "${condition.context?.milestone}" is due in ${condition.measuredValue} day(s), within the ${condition.threshold}-day warning window.`,
        recommendedAction: (condition) => `Confirm with the owning department that milestone "${condition.context?.milestone}" is on track, and record a revised date now if it is not.`,
    },
    {
        ...
        detect: (context) => {
            ...
            return fromLadder('MILESTONE_OVERDUE', overdue, {
                ...
                expression: (severity, threshold) => `milestone_overdue_days = ${overdue} >= ${threshold} (${severity} rung)`,
        ...
        description: (condition, context) => `Milestone "${context.currentMilestone?.name ?? 'current milestone'}" is ${condition.measuredValue} day(s) past its planned date.`,
        recommendedAction: (_condition, context) => `Assign an accountable officer to milestone "${context.currentMilestone?.name ?? 'the overdue milestone'}" and agree a revised completion date.`,
    },
    {
        ...
        detect: (context) => {
            ...
            if (series.length < window.minPoints)
                return null;
            ...
            if (risingShare < (window.minRisingShare ?? 0))
                return null;
            return fromLadder('RISK_TREND_INCREASING', round(delta), {
                unit: 'probability',
                source: 'PREDICTION_HISTORY',
                comparisonValue: round(first.value),
                expression: (severity, threshold) => `delay_probability rose ${round(delta)} across ${series.length} predictions in ${window.days} days >= ${threshold} (${severity} rung)`,
        ...
        description: (condition) => `Predicted delay probability rose from ${condition.context?.from} to ${condition.context?.to} across ${condition.context?.points} predictions in the last ${condition.context?.windowDays} days.`,
        recommendedAction: () => 'Review what changed on the case during this window and confirm whether the open bottlenecks are being worked.',
    },
    {
        ...
        detect: (context) => {
            ...
            return fromLadder('HIGH_DELAY_PROBABILITY', round(probability), {
                unit: 'probability',
                source: 'PREDICTION',
                expression: (severity, threshold) => `delay_probability = ${round(probability)} >= ${threshold} (${severity} rung)`,
                bucket: (severity) => `${severity}`,
                observedAt: context.asOfAt,
                context: {
        ...
        department: projectDepartment,
        description: (condition, context) => `Calibrated delay probability is ${condition.measuredValue} over a ${context.prediction.horizonDays ?? 90}-day horizon, at or above the ${condition.threshold} threshold.`,
        recommendedAction: () => 'Review the ranked recommendations for this project and confirm an owner for the highest-priority open action.',
    },
    {
        ...
        detect: (context) => {
            if (context.prediction.riskLevel !== 'CRITICAL')
                return null;
            return {
                measuredValue: 1,
                unit: 'classification',
        ...
        department: projectDepartment,
        description: (condition) => `The project is classified CRITICAL risk, with a calibrated delay probability of ${condition.context?.delayProbability}.`,
        recommendedAction: () => 'Escalate to the district review and confirm an owner and date for every open critical action on this project.',
    },
    {
        ...
        detect: (context) => {
            ...
            return fromLadder('RISK_SCORE_JUMP', round(delta), {
                ...
                expression: (severity, threshold) => `delay_probability rose ${round(delta)} in one step over ${stepDays} day(s) >= ${threshold} (${severity} rung)`,
                bucket: (severity) => `${severity}`,
                observedAt: last.at,
        ...
        department: projectDepartment,
        description: (condition) => `Predicted delay probability moved from ${condition.context?.from} to ${condition.context?.to} in a single step over ${condition.context?.stepDays} day(s).`,
        recommendedAction: () => 'Check which case values changed since the previous prediction, and confirm the new figures are recorded correctly.',
    },
    {
        ...
        detect: (context) => {
            ...
            if (isNumber(days)) {
                return fromLadder('COMPENSATION_BACKLOG', days, {
                    ...
                    expression: (severity, threshold) => `payment_processing_days = ${days} >= ${threshold} (${severity} rung)`,
                    bucket: (severity) => `${severity}`,
                    observedAt: context.asOfAt,
            ...
            if (context.metrics.compensationApprovalPendingFlag !== true || !isNumber(amount) || amount <= 0)
                return null;
            return {
        ...
        description: (condition) => condition.unit === 'day'
        ...
        recommendedAction: (condition) => condition.unit === 'day'
    ...
    },
    {
        ...
        detect: (context) => {
            ...
            if (!isNumber(current))
                return null;
            const baseline = earliestObservation(context, window.days, (metrics) => metrics.unresolvedObjectionCount ?? metrics.objectionCount);
            if (!baseline)
                return null;
            ...
            return fromLadder('OBJECTIONS_INCREASING', increase, {
                ...
                expression: (severity, threshold) => `unresolved_objection_count rose by ${increase} (${baseline.value} to ${current}) in ${window.days} days >= ${threshold} (${severity} rung)`,
                bucket: (severity) => `${severity}`,
                observedAt: context.asOfAt,
        ...
        description: (condition) => `Unresolved landowner objections rose from ${condition.context?.from} to ${condition.context?.to} over the last ${condition.context?.windowDays} days.`,
        recommendedAction: () => 'Schedule hearings for the newly recorded objections and assign a reviewing officer to each.',
    },
    {
        type: 'LEGAL_ISSUE',
        ...
        detect: (context) => {
            ...
            if (isNumber(count) && count > 0) {
                return fromLadder('LEGAL_ISSUE', count, {
                    unit: 'case',
                    source: 'PROJECT_DATA',
                    expression: (severity, threshold) => `open_legal_case_count = ${count} >= ${threshold} (${severity} rung)`,
                    bucket: (severity) => `${severity}:cases`,
                    observedAt: context.asOfAt,
                });
            }
            if (context.metrics.legalDisputeFlag !== true)
                return null;
            return {
                measuredValue: 1,
                unit: 'dispute',
        ...
        description: (condition) => condition.stateBucket.includes('stay-order')
        ...
        recommendedAction: (condition) => condition.stateBucket.includes('stay-order')
    ...
    },
    {
        ...
        detect: (context) => {
            ...
            if (!isNumber(pending) || pending <= 0)
                return null;
            return fromLadder('DOCUMENT_VERIFICATION_BACKLOG', pending, {
                unit: 'document',
                source: 'PROJECT_DATA',
                comparisonValue: context.metrics.requiredDocumentCount ?? null,
                expression: (severity, threshold) => `document_verification_pending_count = ${pending} >= ${threshold} (${severity} rung)`,
                bucket: (severity) => `${severity}`,
                observedAt: context.asOfAt,
            });
        },
        department: () => ({ name: 'Revenue Department', basis: 'Owner of document verification.' }),
        description: (condition) => `${condition.measuredValue} submitted document(s) are awaiting verification, at or above the ${condition.threshold}-document threshold.`,
    ...
    },
    {
        ...
        detect: (context) => {
            ...
            return fromLadder('DEPARTMENT_WORKLOAD_OVERLOAD', index, {
                ...
                expression: (severity, threshold) => `department_workload_index for ${top.department} = ${index} >= ${threshold} (${severity} rung)`,
        ...
        description: (condition) => `${condition.context?.department} is carrying a workload index of ${condition.measuredValue} across ${condition.context?.openItems ?? 'an unrecorded number of'} open items.`,
        recommendedAction: () => 'Rebalance the case load or add reviewing officers, and confirm which pending items are reassigned.',
    },
];
export const DETECTOR_BY_TYPE = new Map(DETECTORS.map((detector) => [detector.type, detector]));
