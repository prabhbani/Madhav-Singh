 ...
 */
const identifier = (max = 120) => safeText(max, 2).refine((value) => /^[\p{L}\p{M}\p{N} .,'()&/-]+$/u.test(value), 'Identifier contains unsupported characters');
/**
...
const password = z
    ...
    .refine((value) => /[a-z]/.test(value) && /[A-Z]/.test(value) && /[0-9]/.test(value), 'Password must combine lower case, upper case, and digits');
