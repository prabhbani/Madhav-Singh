...
import { idParam, projectList, projectCreate, projectUpdate, milestoneCreate, milestoneUpdate, predictionCreate, acknowledge, documentCreate, login, recommendationGenerate, recommendationStatus, alertEvaluate, alertEvaluateAll, alertList, alertStatus, auditLogList, userCreate, userList, userUpdate, } from '../validators/schemas.js';
const router = Router();
// --- Public -----------------------------------------------------------------
...
router.post('/projects', requirePermission('projects:create'), validate(projectCreate), enforceScopeOnBody('projects', { required: true }), audit('PROJECT_CREATE'), projects.create);
router.put('/projects/:id', requirePermission('projects:update'), validate(projectUpdate), 
...
enforceScope('project'), enforceScopeOnBody('projects'), audit('PROJECT_UPDATE', { subject: 'project' }), projects.update);
router.delete('/projects/:id', requirePermission('projects:delete'), validate(idParam), enforceScope('project'), audit('PROJECT_DELETE', { subject: 'project' }), projects.remove);
...
router.post('/projects/:id/milestones', requirePermission('cases:create'), validate(milestoneCreate), enforceScope('project'), audit('CASE_MILESTONE_CREATE'), projects.createMilestone);
router.put('/milestones/:id', requirePermission('cases:update'), validate(milestoneUpdate), enforceScope('milestone'), audit('CASE_MILESTONE_UPDATE', { subject: 'milestone' }), projects.updateMilestone);
// --- Documents --------------------------------------------------------------
router.post('/documents', requirePermission('documents:create'), validate(documentCreate), enforceScope('project', { from: 'body.projectId', resource: 'documents' }), audit('DOCUMENT_CREATE'), documents.create);
// Reads are audited too: case documents carry landowner records.
router.get('/documents/:id', requirePermission('documents:read'), validate(idParam), enforceScope('document'), audit('DOCUMENT_READ'), documents.get);
// --- Predictions ------------------------------------------------------------
router.post('/predictions', expensiveLimiter, requirePermission('predictions:create'), validate(predictionCreate), enforceScope('project', { from: 'body.projectId', resource: 'predictions' }), audit('PREDICTION_CREATE'), predictions.create);
...
router.post('/alerts/evaluate', expensiveLimiter, requireRole('SUPER_ADMIN', 'STATE_ADMIN'), requirePermission('alerts:evaluate'), validate(alertEvaluateAll), audit('ALERT_EVALUATE_ALL'), alerts.evaluateAll);
router.post('/projects/:id/alerts/evaluate', expensiveLimiter, requirePermission('alerts:evaluate'), validate(alertEvaluate), enforceScope('project', { resource: 'alerts' }), audit('ALERT_EVALUATE'), alerts.evaluate);
router.get('/alerts/:id/history', requirePermission('alerts:read'), validate(idParam), enforceScope('alert'), alerts.history);
router.post('/alerts/:id/acknowledge', requirePermission('alerts:acknowledge'), validate(acknowledge), enforceScope('alert'), audit('ALERT_ACKNOWLEDGE', { subject: 'alert' }), alerts.acknowledge);
router.patch('/alerts/:id/status', requirePermission('alerts:manage'), validate(alertStatus), enforceScope('alert'), audit('ALERT_STATUS_UPDATE', { subject: 'alert' }), alerts.updateStatus);
// --- Recommendations --------------------------------------------------------
router.get('/recommendations/policy-versions', requirePermission('recommendations:read'), recommendations.versions);
router.get('/projects/:id/recommendations', requirePermission('recommendations:read'), validate(idParam), enforceScope('project', { resource: 'recommendations' }), recommendations.list);
router.post('/projects/:id/recommendations', expensiveLimiter, requirePermission('recommendations:generate'), validate(recommendationGenerate), enforceScope('project', { resource: 'recommendations' }), audit('RECOMMENDATION_GENERATE'), recommendations.generate);
router.patch('/recommendations/:id/status', requirePermission('recommendations:manage'), validate(recommendationStatus), enforceScope('recommendation'), audit('RECOMMENDATION_STATUS_UPDATE', { subject: 'recommendation' }), recommendations.updateStatus);
// --- Users ------------------------------------------------------------------
