...
export type NotificationEvent = {
  ...
  /** The condition expression that fired, so a delivered message is auditable. */
  trigger?: string;
...
export const notificationService = {
  ...
  async publishAll(events: NotificationEvent[]): Promise<{ published: number; failed: number }> {
    let published = 0;
    let failed = 0;
