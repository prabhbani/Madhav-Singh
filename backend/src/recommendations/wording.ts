...
const REASON_HEDGES: readonly RegExp[] = [
...
const IMPACT_HEDGES: readonly RegExp[] = [
  /\bmay\b/i,
  /\bcould\b/i,
  /\bis intended to\b/i,
  /\bsupports\b/i,
...
const findBanned = (text: string): string | null => {
...
const hasHedge = (text: string, hedges: readonly RegExp[]): boolean =>
  hedges.some((hedge) => hedge.test(text));

/** Rejects causal or absolute wording in any rendered field. */
export const assertNonCausal = (field: string, text: string): string => {
  const violation = findBanned(text);
  if (violation) throw new WordingViolationError(field, violation, text);
  return text;
};

/** An action is an imperative instruction; it only has to avoid banned wording. */
export const renderAction = (text: string): string => assertNonCausal('action', text.trim());

/** A reason must avoid banned wording and must carry an explicit hedge. */
export const renderReason = (text: string): string => {
...
export const renderImpact = (text: string): string => {
  const trimmed = text.trim().replace(/\s+/g, ' ');
