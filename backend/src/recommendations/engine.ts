...
const PRIORITY_ORDER: Record<Priority, number> = { LOW: 0, MEDIUM: 1, HIGH: 2, CRITICAL: 3 };
const TERMINAL_STATUSES: ReadonlySet<RecommendationStatus> = new Set(['COMPLETED', 'DISMISSED', 'EXPIRED']);
/** Project statuses that close a case to further recommendations. */
const CLOSED_PROJECT_STATUSES: ReadonlySet<string> = new Set(['COMPLETED', 'CANCELLED']);
...
const normalizeFactorCode = (code: string): string =>
  String(code).trim().toUpperCase().replace(/[^A-Z0-9]+/g, '_').replace(/^_|_$/g, '');

const higherPriority = (left: Priority, right: Priority): Priority =>
  PRIORITY_ORDER[left] >= PRIORITY_ORDER[right] ? left : right;

const lowerPriority = (left: Priority, right: Priority): Priority =>
  PRIORITY_ORDER[left] <= PRIORITY_ORDER[right] ? left : right;

const bandFor = (score: number): Priority => {
...
const deriveMilestoneOverdueDays = (
  milestone: MilestoneContext | null,
  metricOverdueDays: number | undefined,
  asOfAt: Date,
): number | null => {
  ...
  if (plannedAt && !completed) {
    ...
    if (overdue > 0) return Math.floor(overdue);
  }
  if (isNumber(metricOverdueDays)) return Math.max(0, metricOverdueDays);
  return null;
};

type FactorIndex = {
  byCode: Map<string, { factor: RiskFactorContext; rank: number; contribution: number }>;
  contributionsSupplied: boolean;
};
 ...
 */
const indexRiskFactors = (factors: RiskFactorContext[]): FactorIndex => {
  const supplied = factors.some((factor) => isNumber(factor.relativeContribution));
  const total = factors.length;
  const decayDenominator = (total * (total + 1)) / 2;
  const byCode = new Map<string, { factor: RiskFactorContext; rank: number; contribution: number }>();
...
type ModelLinkage = {
  score: number;
  related: RelatedRiskFactor | null;
  note: string;
};

const computeModelLinkage = (entry: CatalogEntry, index: FactorIndex): ModelLinkage => {
  const matches: Array<{ factor: RiskFactorContext; rank: number; contribution: number }> = [];
  for (const code of entry.linkedFactorCodes) {
    const match = index.byCode.get(normalizeFactorCode(code));
    if (match && !matches.some((existing) => existing.factor === match.factor)) matches.push(match);
  }
  ...
  const related: RelatedRiskFactor = {
...
type UrgencyResult = { score: number; notes: string[] };
...
type HistoricalResult = { score: number; note: string; usedOutcomeRates: boolean };

const computeHistoricalSupport = (code: EvidenceCode, context: ExtractionContext): HistoricalResult => {
...
type PriorityDecision = { priority: Priority; basis: 'RANKING_SCORE' | 'POLICY_OVERRIDE'; overrides: string[] };
...
const decidePriority = (
  ...
  if (
    input.prediction.riskLevel === 'CRITICAL' &&
    related !== null &&
    related.rank !== null &&
    ...
    if (raised !== priority) {
      overrides.push(
        `CRITICAL_RISK_TOP_FACTOR: project risk level is CRITICAL and this action addresses model factor #${related.rank}; priority raised to at least HIGH.`,
      );
      priority = raised;
    }
  }
  ...
  const degraded =
    input.prediction.predictionStatus === 'RULE_ONLY_FALLBACK' ||
  ...
  if (degraded && !CODE_POLICY[entry.code].hardStop) {
    ...
    if (capped !== priority) {
      overrides.push(
        `DEGRADED_PREDICTION_CAP: prediction status is ${input.prediction.predictionStatus ?? 'OK'} with confidence band ${input.prediction.confidenceBand ?? 'UNKNOWN'}; priority capped at HIGH.`,
      );
      priority = capped;
    }
  }
...
const computeDeadline = (
  entry: CatalogEntry,
  priority: Priority,
  workload: DepartmentWorkload | undefined,
  asOfAt: Date,
  targetDate: Date | null,
): SuggestedDeadline => {
...
): { status: RecommendationStatus; basis: string } => {
  const previous = existing.get(code);
  if (!previous) return { status: 'OPEN', basis: 'Newly generated from evidence at this snapshot.' };
...
export type EngineOptions = {
  /** Overrides the generation timestamp; supply it to keep output reproducible. */
...
export const generateRecommendations = (
  ...
  const pendingTaskByCode = new Map<string, PendingTask>();
  for (const task of pendingTasks) {
    if (task?.taskCode) pendingTaskByCode.set(normalizeFactorCode(task.taskCode), task);
  }
  const workloadByDepartment = new Map<string, DepartmentWorkload>();
  for (const workload of workloads) {
    if (workload?.department) workloadByDepartment.set(workload.department, workload);
  }
  const existingByCode = new Map<string, ExistingRecommendation>();
  for (const existing of input.existingRecommendations ?? []) {
    if (existing?.evidenceCode) existingByCode.set(normalizeFactorCode(existing.evidenceCode), existing);
  }
  ...
  const predictedRisk = clamp(
    isNumber(input.prediction.riskScore) ? input.prediction.riskScore : input.prediction.delayProbability,
  );
  ...
  const scored: RecommendedAction[] = [];

  // A completed or cancelled case generates no new work. Counters left on a
  // closed record are history, not a bottleneck, and putting them back on an
  // officer's queue is how a closed case gets reopened by accident.
  const projectClosed = CLOSED_PROJECT_STATUSES.has((input.project.status ?? '').toUpperCase());
  if (projectClosed) {
    for (const entry of CATALOG) {
      suppressed.push({
        evidenceCode: entry.code,
        reason: 'PROJECT_CLOSED',
        detail: `Project status is ${input.project.status}; no action is raised against a closed case.`,
      });
    }
  }

  for (const entry of projectClosed ? [] : CATALOG) {
    let partial;
    try {
      ...
      suppressed.push({
        ...
        detail: `Evidence extraction failed: ${(error as Error).message}`,
      });
      continue;
    }
    if (!partial) {
    ...
    const evidence: EvidenceObservation = buildObservation(entry, partial, context);
    ...
    const rankingScore = round(
      RANKING_WEIGHTS.evidenceSeverity * evidence.severity +
    ...
    const relatedRiskFactor: RelatedRiskFactor = linkage.related ?? {
    ...
    const capacityNote =
      workload && isNumber(workload.workloadIndex) && workload.workloadIndex >= ENGINE_LIMITS.workloadEscalationThreshold
  ...
  scored.sort((left, right) => {
    ...
    const byHardStop =
      Number(CODE_POLICY[right.evidence.code].hardStop ?? false) - Number(CODE_POLICY[left.evidence.code].hardStop ?? false);
    if (byHardStop !== 0) return byHardStop;
  ...
  const missingInputBlocks: string[] = [];
  if (riskFactors.length === 0) missingInputBlocks.push('topRiskFactors');
  if (!input.historicalPatterns) missingInputBlocks.push('historicalPatterns');
  if (pendingTasks.length === 0) missingInputBlocks.push('pendingTasks');
  if (workloads.length === 0) missingInputBlocks.push('departmentWorkload');
  if (Object.keys(metrics).length === 0) missingInputBlocks.push('project.metrics');
  if (!milestone) missingInputBlocks.push('currentMilestone');
  ...
  if (projectClosed) {
    warnings.push(`Project status is ${input.project.status}; every detector was skipped.`);
  }

  const byPriority: Record<Priority, number> = { CRITICAL: 0, HIGH: 0, MEDIUM: 0, LOW: 0 };
  for (const recommendation of emitted) byPriority[recommendation.priority] += 1;
...
export const traceabilityFor = (
  recommendation: RecommendedAction,
  result: RecommendationResult,
  prediction: { modelVersion?: string; predictionId?: string },
) => ({
