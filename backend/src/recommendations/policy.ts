...
import type { EvidenceCode, Priority } from './types.js';
...
export const PRIORITY_BANDS: ReadonlyArray<{ min: number; priority: Priority }> = [
...
export const DEADLINE_URGENCY_FACTOR: Record<Priority, number> = {
  CRITICAL: 0.4,
  HIGH: 0.65,
  MEDIUM: 1,
  LOW: 1.25,
...
/** Per-code policy configuration: base importance, SLA, and materiality. */
export type CodePolicy = {
  /** Approved base importance in [0,1]; feeds the ranking score directly. */
  weight: number;
  /** Departmental service window in days before urgency adjustment. */
  slaDays: number;
  /** Measured value at which the evidence is considered fully material. */
  threshold: number;
  /** Set only for governed hard-stop conditions. */
  hardStop?: boolean;
};

export const CODE_POLICY: Record<EvidenceCode, CodePolicy> = {
...
export const ratioSeverity = (value: number, threshold: number): number =>
  threshold <= 0 ? (value > 0 ? 1 : 0) : clamp(value / threshold);
 ...
 */
export const sigmoidSeverity = (value: number, tolerance: number, scale: number): number => {
...
export const baselineRatioSeverity = (value: number, baseline: number): number =>
  baseline <= 0 ? 0 : clamp(value / baseline - 1);

/** Binary condition, used only for governed flags such as a stay order. */
export const flagSeverity = (flag: boolean): number => (flag ? 1 : 0);
