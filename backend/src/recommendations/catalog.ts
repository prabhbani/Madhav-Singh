...
export type ExtractionContext = {
  ...
  pendingTaskByCode: Map<string, PendingTask>;
  workloadByDepartment: Map<string, DepartmentWorkload>;
  historicalPatterns: HistoricalPatterns | null;
  /** Derived once, from the milestone plan date or a caller-supplied value. */
  milestoneOverdueDays: number | null;
};

/** What an extractor returns; shared fields are filled in by `buildObservation`. */
...
export type CatalogEntry = {
  code: EvidenceCode;
  label: string;
  /** Dataset/feature columns this evidence is read from. */
  featureCodes: string[];
  /** Model or rule factor codes that represent the same operational issue. */
  linkedFactorCodes: string[];
  impactCategory: string;
  extract: (context: ExtractionContext) => PartialObservation | null;
  department: (context: ExtractionContext) => DepartmentAssignment;
  action: (evidence: EvidenceObservation, context: ExtractionContext) => string;
  reason: (evidence: EvidenceObservation, context: ExtractionContext) => string;
  impact: (evidence: EvidenceObservation, context: ExtractionContext) => string;
};
...
): { value: number; source: EvidenceSource; task?: PendingTask } | null => {
  const task = context.pendingTaskByCode.get(taskCode);
  if (task && isNumber(task.count)) return { value: task.count, source: 'PENDING_TASKS', task };
  if (isNumber(metricValue)) return { value: metricValue, source: 'PROJECT_DATA' };
  return null;
};

const unitFor = (task: PendingTask | undefined, fallback: string): string => task?.unit ?? fallback;
...
const milestoneOwnerOrProject = (context: ExtractionContext): DepartmentAssignment => {
  const owner = context.milestone?.ownerDepartment;
  if (owner) return { name: owner, basis: 'Recorded owner department of the current milestone.' };
  return { name: context.project.department, basis: 'Owning department of the project; no milestone owner is recorded.' };
};

const medianResolution = (patterns: HistoricalPatterns | null, code: EvidenceCode): number | null => {
...
const historicalClause = (context: ExtractionContext, code: EvidenceCode): string => {
...
export const CATALOG: readonly CatalogEntry[] = [
  {
    ...
    extract: (context) => {
      if (context.metrics.stayOrderFlag !== true) return null;
      return {
        measuredValue: 1,
        unit: 'order',
    ...
    action: () =>
      'Refer the active stay order to the legal department for review of the current position and next hearing, and hold any acquisition step that the order restrains.',
    reason: () =>
      `A stay order is recorded as active against this acquisition, and an active stay ${ASSOCIATION_CLAUSE}. Legal referral is recommended because the order governs which acquisition steps may lawfully proceed.`,
    impact: () =>
      'May clarify the permitted scope of work and could help mitigate the risk of an acquisition step being taken contrary to the order.',
  },
  {
    ...
    department: (context) =>
      departmentFrom(context, 'OWNERSHIP_UNRESOLVED', 'Land Records Department', 'Statutory custodian of title and mutation records.'),
    action: (evidence) =>
      `Complete ownership verification for the ${formatCount(evidence.measuredValue, evidence.unit)} with unresolved records.`,
    reason: (evidence, context) =>
      `Ownership verification is currently incomplete for ${formatCount(evidence.measuredValue, evidence.unit)} and ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'OWNERSHIP_UNRESOLVED')}`,
    impact: () =>
      'May reduce the administrative bottleneck in title verification and could help mitigate downstream compensation and possession delays.',
  },
  {
    ...
    extract: (context) => {
      ...
      if (ageDays === null && !approvalPending) return null;
      if (ageDays === null) {
        return {
    ...
    department: (context) =>
      departmentFrom(context, 'COMPENSATION_PENDING', 'Finance and Compensation Department', 'Statutory owner of compensation assessment and disbursement.'),
    action: (evidence) =>
      evidence.unit === 'day'
  ...
  },
  {
    ...
    department: (context) =>
      departmentFrom(context, 'PENDING_APPROVAL_COUNT', context.project.department, 'Owning department of the project approval queue.'),
    action: (evidence) =>
      `Route the ${formatCount(evidence.measuredValue, 'pending approval')} to the authorized decision-maker and confirm a decision date for each.`,
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, 'approval')} remain pending against a review threshold of ${evidence.threshold}, and an ageing approval queue ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'PENDING_APPROVAL_COUNT')}`,
    impact: () =>
      'May reduce the administrative bottleneck in the approval queue and could help mitigate stage stagnation.',
  },
  {
    ...
    action: (_evidence, context) =>
      `Assign an accountable officer to the milestone "${context.milestone?.name ?? 'current milestone'}" and agree a revised completion date at the next review.`,
    reason: (evidence, context) => {
    ...
    impact: () =>
      'May restore schedule ownership for the open milestone and could help mitigate further slippage into the next stage.',
  },
  {
    ...
    extract: (context) => {
      const resolved = resolveCount(
        ...
        context.metrics.unresolvedObjectionCount ?? context.metrics.objectionCount,
      );
      if (!resolved || resolved.value <= 0) return null;
    ...
    department: (context) =>
      departmentFrom(context, 'OPEN_OBJECTION_COUNT', 'Land Acquisition Cell', 'Owner of objection hearings and stakeholder resolution.'),
    action: (evidence) =>
      `Schedule hearings for the ${formatCount(evidence.measuredValue, 'unresolved objection')} and assign a reviewing officer to each.`,
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, 'landowner objection')} remain unresolved against a review threshold of ${evidence.threshold}, and open objection volume ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'OPEN_OBJECTION_COUNT')}`,
    impact: () =>
      'May reduce the unresolved stakeholder workload and could help mitigate escalation of objections into legal disputes.',
  },
  {
    ...
    action: (evidence) =>
      `Assign legal representation and a hearing calendar for the ${formatCount(evidence.measuredValue, evidence.unit)} currently open.`,
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, evidence.unit)} are open against this acquisition, and open legal exposure ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'OPEN_LEGAL_CASE_COUNT')}`,
    impact: () =>
      'May give the litigation a tracked hearing calendar and could help mitigate procedural delay from unrepresented matters.',
  },
  {
    ...
    department: (context) =>
      departmentFrom(context, 'MISSING_DOCUMENT_COUNT', 'Revenue Department', 'Owner of the acquisition document checklist.'),
    action: (evidence) =>
      `Complete the document checklist for the ${formatCount(evidence.measuredValue, 'missing required document')} and record each submission.`,
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, 'required document')} are missing from the case file, and an incomplete document set ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'MISSING_DOCUMENT_COUNT')}`,
    impact: () =>
  ...
  },
  {
    ...
    department: (context) =>
      departmentFrom(context, 'INVALID_DOCUMENT_COUNT', 'Revenue Department', 'Owner of document verification and re-submission.'),
    action: (evidence) =>
      `Obtain valid replacements for the ${formatCount(evidence.measuredValue, 'rejected or expired document')} and re-submit them for verification.`,
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, 'document')} are recorded as rejected or expired, and invalid supporting evidence ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'INVALID_DOCUMENT_COUNT')}`,
    impact: () =>
      'May restore a valid evidence set for the case and could help mitigate rejection at the next approval step.',
  },
  {
    ...
    extract: (context) => {
      const resolved = resolveCount(
        ...
        context.metrics.documentVerificationPendingCount,
      );
      if (!resolved || resolved.value <= 0) return null;
    ...
    department: (context) =>
      departmentFrom(context, 'DOCUMENT_VERIFICATION_PENDING', 'Revenue Department', 'Owner of document verification.'),
    action: (evidence) =>
      `Clear the verification backlog of ${formatCount(evidence.measuredValue, 'submitted document')}.`,
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, 'submitted document')} are awaiting verification against a queue threshold of ${evidence.threshold}, and an ageing verification backlog ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'DOCUMENT_VERIFICATION_PENDING')}`,
    impact: () =>
      'May reduce the verification backlog and could help mitigate downstream approval waiting time.',
  },
  {
    ...
    department: (context) =>
      departmentFrom(context, 'BLOCKED_DEPENDENCY_COUNT', 'District Administration', 'Coordinating authority for inter-department dependencies.'),
    action: (evidence) =>
      `Convene a coordination review for the ${formatCount(evidence.measuredValue, 'blocked dependency')} and record an owner and unblock date for each.`,
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, 'inter-department dependency')} are blocked against a threshold of ${evidence.threshold}, and blocked dependencies ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'BLOCKED_DEPENDENCY_COUNT')}`,
    impact: () =>
      'May give each blocked dependency a named owner and could help mitigate idle waiting between departments.',
  },
  {
    ...
    reason: (evidence, context) =>
      `${formatCount(evidence.measuredValue, 'milestone')} have already been missed or rescheduled, and repeated slippage ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'MILESTONE_SLIPPAGE_COUNT')}`,
    impact: () =>
      'May surface the recurring schedule constraint at project level and could help mitigate continued drift across stages.',
  },
  {
    code: 'STAGE_AGING',
    ...
    extract: (context) => {
      ...
      const baseline =
        context.metrics.averageStageDurationDays ?? context.historicalPatterns?.averageStageDurationDays ?? null;
    ...
    department: milestoneOwnerOrProject,
    action: (_evidence, context) =>
      `Review the bottleneck in the "${context.milestone?.stage ?? context.milestone?.name ?? 'current'}" stage and assign an owner to the activity holding it open.`,
    reason: (evidence, context) =>
      `The current stage has been open for ${formatCount(evidence.measuredValue, 'day')} against a baseline of ${formatCount(evidence.comparisonValue ?? 0, 'day')}, and stage age beyond its baseline ${ASSOCIATION_CLAUSE}.${historicalClause(context, 'STAGE_AGING')}`,
    impact: () =>
      'May identify the specific activity holding the stage open and could help mitigate further stagnation.',
  },
  {
    ...
    extract: (context) => {
      let saturated: DepartmentWorkload | null = null;
      for (const workload of context.workloadByDepartment.values()) {
        if (!isNumber(workload.workloadIndex)) continue;
        if (workload.workloadIndex < CODE_POLICY.DEPARTMENT_CAPACITY.threshold) continue;
        if (!saturated || workload.workloadIndex > saturated.workloadIndex) saturated = workload;
      }
    ...
    action: (evidence) =>
      `Rebalance the case load or add reviewing officers where the workload index stands at ${evidence.measuredValue.toFixed(2)}, and confirm which pending items are reassigned.`,
    reason: (evidence) =>
      `${evidence.description} A department workload index at or above the capacity threshold ${ASSOCIATION_CLAUSE}, and rebalancing is recommended because the actions above are assigned to a queue already at capacity.`,
    impact: () =>
      'May free reviewing capacity for the higher-priority actions on this project and could help mitigate queueing delay.',
  },
  {
    ...
    extract: (context) => {
      ...
      if (!isNumber(days) || days <= 0) return null;
    ...
    action: (evidence) =>
      `Refresh the case record, which has not been updated for ${formatCount(evidence.measuredValue, 'day')}, and confirm the current stage, documents, and objection counts.`,
    reason: (evidence) =>
      `${evidence.description} A stale record ${ASSOCIATION_CLAUSE} and also lowers the confidence of every other item on this list, so an update is recommended because the remaining recommendations depend on it.`,
    impact: () =>
      'May improve the evidence quality behind this assessment and could help surface bottlenecks that current data does not yet show.',
  },
];

export const CATALOG_BY_CODE: ReadonlyMap<EvidenceCode, CatalogEntry> = new Map(
  CATALOG.map((entry) => [entry.code, entry]),
);

/** Kept as an explicit export so unused-import checks do not hide the contract. */
export type { PredictionContext, ProjectContext, ProjectMetrics, MilestoneContext, PendingTask };
