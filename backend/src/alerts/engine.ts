...
import { DETECTORS, type DetectedCondition, type DetectionContext, type Detector } from './detectors.js';
import {
  ALERT_POLICY_VERSION,
  COOLDOWN_HOURS,
  DEGRADED_PREDICTION_SEVERITY_CAP,
  DETECTOR_VERSION,
  MODEL_DERIVED_DETECTORS,
  NOTIFICATION_POLICY,
  SEVERITY_ORDER,
  lowerSeverity,
} from './policy.js';
...
/** Project statuses that close a case to further warnings. */
...
const BASE_ALERT_LIMITATIONS: readonly string[] = [
...
const buildDetectionContext = (input: AlertEvaluationInput, asOfAt: Date): DetectionContext => {
  ...
  const upcomingMilestones = (input.upcomingMilestones ?? [])
    ...
    .filter((entry): entry is { milestone: MilestoneContext; plannedDate: Date } => entry.plannedDate !== null)
  ...
  const historyPoints = (input.predictionHistory ?? [])
    .map((point: PredictionHistoryPoint) => {
    ...
    .filter((point): point is PredictionHistoryPoint & { at: Date; value: number } => point !== null)
  ...
  const observationHistory = (input.observationHistory ?? [])
    .map((point: ObservationHistoryPoint) => {
      const at = toDate(point.observedAt);
      return at ? { ...point, at } : null;
    })
    .filter((point): point is ObservationHistoryPoint & { at: Date } => point !== null)
    .sort((left, right) => left.at.getTime() - right.at.getTime());

  const workloadByDepartment = new Map<string, NonNullable<AlertEvaluationInput['departmentWorkload']>[number]>();
  ...
  const officersByDepartment = new Map<string, OfficerAssignment>();
...
type GovernedSeverity = { severity: AlertSeverity; basis: 'THRESHOLD_LADDER' | 'POLICY_OVERRIDE'; overrides: string[] };

const governSeverity = (
  detector: Detector,
  ...
  if (condition.override) overrides.push(condition.override);
  ...
  if (degraded && MODEL_DERIVED_DETECTORS.includes(detector.type)) {
    ...
    if (capped !== severity) {
      overrides.push(
        `DEGRADED_PREDICTION_CAP: prediction status ${status} with confidence band ${input.prediction.confidenceBand ?? 'UNKNOWN'}; severity capped at ${DEGRADED_PREDICTION_SEVERITY_CAP}.`,
      );
      severity = capped;
    }
  }
...
export const evaluateAlerts = (
  ...
  const existingByType = new Map<string, ExistingAlert>();
  for (const existing of input.existingAlerts ?? []) {
    if (existing?.alertType) existingByType.set(String(existing.alertType), existing);
  }
  ...
  // A completed or cancelled case raises no new warning, and anything still
  // open against it is resolved: the condition may persist in the data, but it
  // is no longer something an officer can or should act on.
    ...
    for (const detector of DETECTORS) {
      ...
      if (existing && LIVE_STATUSES.has(existing.status)) {
        resolved.push({
          ...
          reason: `PROJECT_CLOSED: project status is ${input.project.status}.`,
        });
      } else {
        suppressed.push({
          alertType: detector.type,
          reason: 'PROJECT_CLOSED',
          detail: `Project status is ${input.project.status}; no warning is raised against a closed case.`,
          severity: null,
        });
      }
    }
  ...
  // --- Pass 1: detection -----------------------------------------------------
  for (const detector of projectClosed ? [] : DETECTORS) {
    let condition: DetectedCondition | null;
    try {
  ...
  const strongestInGroup = new Map<string, Candidate>();
  for (const candidate of candidates) {
    ...
    if (bySeverity > 0 || (bySeverity === 0 && byPrecedence > 0)) strongestInGroup.set(group, candidate);
  }
  ...
  for (const candidate of retained) {
    ...
    let decision: DetectedAlert['decision'];
    let decisionReason: string;
    let notify = false;
    let notifyBasis: string;
    ...
    if (input.prediction.confidenceBand === 'LOW') limitations.push('Prediction confidence for this project is LOW.');
  ...
  for (const alert of notifying) {
    if (exempt.has(alert.severity)) continue;
    if (spent < budget) {
      spent += 1;
      continue;
    }
  ...
  alerts.sort((left, right) => {
    ...
    if (left.notify !== right.notify) return left.notify ? -1 : 1;
    return left.alertType.localeCompare(right.alertType);
  });

  const missingInputBlocks: string[] = [];
  if ((input.predictionHistory ?? []).length === 0) missingInputBlocks.push('predictionHistory');
  if ((input.observationHistory ?? []).length === 0) missingInputBlocks.push('observationHistory');
  if ((input.departmentWorkload ?? []).length === 0) missingInputBlocks.push('departmentWorkload');
  if ((input.responsibleOfficers ?? []).length === 0) missingInputBlocks.push('responsibleOfficers');
  if ((input.upcomingMilestones ?? []).length === 0) missingInputBlocks.push('upcomingMilestones');
  if (Object.keys(context.metrics).length === 0) missingInputBlocks.push('project.metrics');
  ...
  if (predictionStatus !== 'OK') warnings.push(`Prediction status is ${predictionStatus}; model-derived detectors are capped.`);
  if (dataOrigin === 'SYNTHETIC_DEMO') warnings.push('Project data origin is SYNTHETIC_DEMO.');
  if (projectClosed) warnings.push(`Project status is ${input.project.status}; every detector was skipped.`);

  const bySeverity: Record<AlertSeverity, number> = { INFO: 0, WARNING: 0, HIGH: 0, CRITICAL: 0 };
  for (const alert of alerts) bySeverity[alert.severity] += 1;
  const count = (decision: DetectedAlert['decision']) => alerts.filter((alert) => alert.decision === decision).length;
...
export const alertTypes = (): AlertType[] => DETECTORS.map((detector) => detector.type);
