...
export type ScopeLevel = 'GLOBAL' | 'STATE' | 'DISTRICT' | 'DEPARTMENT' | 'NONE';

/** The widest scope each role may ever hold. */
export const ROLE_SCOPE_LEVEL: Record<Role, Exclude<ScopeLevel, 'NONE'>> = {
  SUPER_ADMIN: 'GLOBAL',
  STATE_ADMIN: 'STATE',
...
export type AccessScope = {
  ...
  districtCode?: string;
  department?: string;
  /** Why the scope came out as it did; surfaced on denials and in audit rows. */
  basis: string;
};
 ...
 */
const sameValue = (left: string | null | undefined, right: string | null | undefined): boolean => {
  const a = clean(left);
  const b = clean(right);
...
export const resolveScope = (user: ScopedUser): AccessScope => {
 ...
 */
export const projectScopeWhere = (scope: AccessScope): Prisma.ProjectWhereInput => {
  switch (scope.level) {
    case 'GLOBAL':
      return {};
    case 'STATE':
...
export const nestedProjectScopeWhere = (scope: AccessScope): { project?: Prisma.ProjectWhereInput } | { id: { in: [] } } =>
  scope.level === 'NONE' ? { id: { in: [] } } : { project: projectScopeWhere(scope) };

/** True when the subject row falls inside the scope. */
export const isInScope = (scope: AccessScope, subject: ScopeSubject): boolean => {
  switch (scope.level) {
    case 'GLOBAL':
      return true;
    case 'STATE':
    ...
    case 'DISTRICT':
      return (
        sameValue(scope.districtCode, subject.district) &&
        (scope.stateCode === undefined || sameValue(scope.stateCode, subject.state))
      );
    case 'DEPARTMENT':
      return (
        sameValue(scope.department, subject.department) &&
 ...
 */
export const assertInScope = (scope: AccessScope, subject: ScopeSubject, resource: string): void => {
  if (isInScope(scope, subject)) return;
...
export const auditScopeWhere = (scope: AccessScope): Prisma.AuditLogWhereInput => {
  if (scope.level === 'GLOBAL') return {};
  if (scope.stateCode) return { actor: { stateCode: insensitive(scope.stateCode) } };
  return { id: { in: [] } };
};
 ...
 * refused.
 */
export const assertBodyFieldsInScope = (
  scope: AccessScope,
  fields: { state?: string | null; district?: string | null; department?: string | null },
  resource: string,
): void => {
  if (scope.level === 'GLOBAL') return;
  if (scope.level === 'NONE') {
  ...
  if (state !== undefined && scope.stateCode !== undefined && !sameValue(scope.stateCode, state)) refuse('state');
  ...
  if (district !== undefined && scope.districtCode !== undefined && !sameValue(scope.districtCode, district)) refuse('district');
