...
export const roleHasPermission = (role: Role, permission: Permission): boolean =>
  PERMISSION_SETS.get(role)?.has(permission) ?? false;

export const roleHasAllPermissions = (role: Role, permissions: readonly Permission[]): boolean =>
  permissions.every((permission) => roleHasPermission(role, permission));

export const roleHasAnyPermission = (role: Role, permissions: readonly Permission[]): boolean =>
...
export const permissionMatrix = () => ({
  ...
  roles: Object.fromEntries(
    (Object.keys(ROLE_PERMISSIONS) as Role[]).map((role) => [role, ROLE_PERMISSIONS[role]]),
  ),
});
