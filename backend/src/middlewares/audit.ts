...
import { AUDIT_POLICY, type AuditActionName } from '../audit/auditPolicy.js';
...
export type AuditOptions = {
  ...
  idFrom?: string;
  /** Resource kind used to load the "before" snapshot. */
  subject?: SubjectKind;
  /**
   * Resolves the id when the response creates the row, so a create can record
   * the identifier the handler just generated.
   */
  idFromResponse?: (body: unknown) => string | undefined;
};
...
const loadBefore = async (subject: SubjectKind, id: string): Promise<unknown> => {
  switch (subject) {
    case 'project':
...
const defaultIdFromResponse = (body: unknown): string | undefined => {
...
export const audit =
  (action: AuditActionName, options: AuditOptions = {}): RequestHandler =>
